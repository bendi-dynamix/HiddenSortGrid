...
import { CellHandle, CellProps, clearElementSelection, emptyText, emptyValue, emptyValueStr, numberToBool, selectElementText } from "../Helper";
import { FieldMetadata } from "../MetadataHelper";

export interface BoolCellProps extends CellProps {
    valuesList: Xrm.OptionSetValue[];
    fieldInfo: FieldMetadata | null;
    onOpenButtonClicked: () => void;
}

export const BoolCell = React.forwardRef<CellHandle, BoolCellProps>(function BoolCellCell(
    {
        rawValue,
        formattedValue,
        isEditing,
    ...
    const optionsList = React.useMemo(() => [...valuesList], [valuesList]);
    ...
    const commit = async (value: string) => {
        ...
        if ((parsed ?? emptyValue) !== (originalRef.current ?? emptyValue))
            await onCommit(numberToBool(parsed), nextFormatted);
    };
