...
export interface MoneyCellProps extends CellProps {
...
export const MoneyCell = React.forwardRef<CellHandle, MoneyCellProps>(function MoneyCell(
    ...
    const commit = async (value: string) => {
        const parsed = extractNumberD365(value, currencySymbol, userFormatInfo, numericInfo) ?? null;
        const nextFormatted =
            error != null
                ? value
                : (formatNumberD365(parsed, currencySymbol, userFormatInfo, numericInfo) ?? "");
    ...
    function validate() {
        if ((currencySymbol ?? "") === "" || numericInfo == null)
            return null;

        const extracted = extractNumberD365(currentValue, currencySymbol, userFormatInfo, numericInfo);
        if (extracted === undefined) {
            const minValueFormatted = formatNumberD365(numericInfo?.minValue, currencySymbol, userFormatInfo, numericInfo);
            const maxValueFormatted = formatNumberD365(numericInfo?.maxValue, currencySymbol, userFormatInfo, numericInfo);

            return `Enter an amount between ${minValueFormatted} and ${maxValueFormatted}`;
        }
